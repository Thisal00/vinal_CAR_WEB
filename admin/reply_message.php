...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  if (empty($phone) || empty($reply)) {
    ...
    $clean_phone = '94' . ltrim($phone, '0'); // Convert to international format
    $whatsapp_message = urlencode("Hello {$data['customer_name']}, your part request has been replied:\n\n$reply");
    $whatsapp_link = "https://wa.me/$clean_phone?text=$whatsapp_message";

    // ✅ Feedback with WhatsApp link
    $feedback = '<div style="padding:20px;color:limegreen;">Reply saved for '.$data['customer_name'].'</div>';
...
<body>
  <div class="container">
    ...
    <p><strong>Part:</strong> <?= htmlspecialchars($data['part_name']) ?> (ID: <?= $data['part_id'] ?>)</p>
    <p><strong>Customer:</strong> <?= htmlspecialchars($data['customer_name']) ?></p>
    <p><strong>Phone:</strong> <?= htmlspecialchars($data['phone']) ?></p>
    <p><strong>Email:</strong> <?= htmlspecialchars($data['email']) ?: '<em>Not provided</em>' ?></p>
